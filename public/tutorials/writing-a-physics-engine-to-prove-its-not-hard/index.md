...
- The `aabb_y_check` must be done within an `elseif` branch, rather than ending the first if and starting a new if. If this is not done, then after resolving the collision on the Y, the velocity will be reset and previous method may return an incorrect location, causing the X collision resolution to trigger as well.
- If you are not resetting the player velocity to 0 on the respective axis after collision, you will move/fall fall through the platform after your velocity exceeds the width/height of the platform, as it will move past the entire platform in a single frame, skipping the AABB checks. 
